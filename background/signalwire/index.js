const auth = {
    username: process.env.SW_PROJECT_ID, // Project-ID
    password: process.env.SW_API_TOKEN // API token
};
const apiurl = `https://${process.env.SW_NAMESPACE}.signalwire.com/api/video`;
...
async function start(port) {
    if (await CreateRoomIfItDoesntExist(ROOMNAME))
        app.listen(port, () => {
            ...
            if(vectorlyToken)  open(`http://localhost:${port}/index.html?vectorlyToken=${vectorlyToken}`);
